...
### Current Free Edition limitation

The deployed endpoint and all four tools were verified on 2026-08-08 through the MCP protocol.
This workspace's current Agent Bricks UI only auto-discovers Databricks Apps whose immutable
name starts with `mcp-`; the required reused name is `ai-support-app`. The external MCP flow also
requires a Unity Catalog location and durable authentication. Because this assignment explicitly
preserves and repoints the existing App instead of deleting/recreating it, Agent Bricks attachment
could not be completed safely. A temporary one-day token tested during diagnosis was immediately
revoked and deleted; no token is present in the repository or submission archive.
